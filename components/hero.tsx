...
const STATS = [
  { k: "01", v: "Solo developer" },
  { k: "02", v: "Projects in progress" },
  { k: "100%", v: "Free estimates" },
  { k: "MG", v: "Antananarivo, Madagascar" },
]
  ...
  return (
    <section
      ...
      className={cn(
        ...
        "yuta-grain bg-background text-foreground"
      )}
    >
      {/* === Backdrop layers === */}
      <DotPattern
        className="absolute inset-0 -z-10 h-full w-full [mask-image:radial-gradient(ellipse_at_top,black_25%,transparent_80%)] fill-foreground/15"
        width={28}
        height={28}
        cr={0.7}
      />
      ...
      <div className="relative mx-auto flex w-full max-w-[1480px] flex-1 flex-col gap-8 px-6 pt-28 pb-12 md:px-10 md:pt-36 md:pb-16">
        ...
        <div className="flex items-end justify-between gap-6">
          <div className="flex-1">
            <span className="text-eyebrow block">— YUTA · MG · 00</span>
            <h1 className="mt-4 font-display text-[clamp(3.5rem,11vw,10rem)] leading-[0.86] font-extrabold tracking-[-0.045em]">
              <span className="block">Build</span>
              <span className="block">
                <span className="relative inline-block">
                  <span className="relative z-10 text-foreground italic">
                    what
                  </span>
                  <span
                    aria-hidden
        ...
        <div className="mt-2 grid items-end gap-8 md:grid-cols-12">
          <p className="max-w-2xl text-[17px] leading-relaxed text-foreground/85 md:col-span-7">
            YUTA Corporation is just me — a solo developer based in
            Antananarivo, Madagascar. I build projects I believe in, one at a
            time. My hope? That in ten years, this company grows strong enough
            to help my country where it matters most.
          </p>
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-[1480px] items-center justify-between gap-4 px-6 pb-8 font-mono text-[10.5px] tracking-[0.28em] text-foreground/60 uppercase md:px-10 md:pb-10">
        <span className="flex items-center gap-3">
          <ArrowDown className="size-3 animate-bounce" />
          Scroll — there&apos;s more below
        </span>
...
export function StatsStrip() {
  return (
    <section
      aria-label="A few honest numbers"
      ...
      <div className="mx-auto grid max-w-[1480px] grid-cols-2 gap-y-6 px-6 py-10 md:grid-cols-4 md:px-10">
        {STATS.map((s) => (
          <div key={s.v} className="flex flex-col gap-1">
            <span className="font-display text-4xl leading-none font-bold tracking-tight md:text-5xl">
              {s.k}
            </span>
            <span className="font-mono text-[10.5px] tracking-[0.22em] text-foreground/75 uppercase">
              {s.v}
            </span>
          </div>
        ))}
